...
def main():
    ...
    parser.add_argument("--do_train", type=bool, default=True, help="是否训练模型")
    ...
    parser.add_argument("--train_batch_size", type=int, default=2, help="每个设备的训练批次大小")
    parser.add_argument("--eval_batch_size", type=int, default=2, help="每个设备的评估批次大小")
    parser.add_argument("--gradient_accumulation", type=int, default=8, help="梯度累积步数")
    parser.add_argument("--eval_accumulation", type=int, default=1, help="评估梯度累积步数")
    ...
    train_dataset = data_utils.load_and_process_data(
        args.train_data_path,
        tokenizer,
        data_utils.format_instruction_basic
    )
    train_dataset = data_utils.process_to_dict(train_dataset)

    print(f"加载验证数据: {args.eval_data_path}")
    eval_dataset = data_utils.load_and_process_data(
        args.eval_data_path,
        tokenizer,
        data_utils.format_instruction_basic
    )
    # 仅评估前 100 条数据
    eval_dataset = data_utils.process_to_dict(eval_dataset.select(range(min(100, len(eval_dataset)))))
    ...
    sft_config = SFTConfig(
        ...
        eval_accumulation_steps=args.eval_accumulation,
        ...
        save_strategy="steps",
        save_steps=10,
        save_total_limit=2,             # 只保留最新的 2 个模
        load_best_model_at_end=True,    # 训练结束时加载验证集表现最好的权重
        metric_for_best_model="loss",   # 以 loss 最低作为评判标准
